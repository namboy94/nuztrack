...
class TeamMemberSwitchEventServiceTest {
    ...
    @Test
    fun getAllEvents() {
        whenever(repository.findAllByEventType(EventType.PARTY_MEMBER_SWITCH)).thenReturn(events)
        assertThat(service.getAllTeamMemberSwitchEvents()).isEqualTo(events)
        verify(repository, times(1)).findAllByEventType(EventType.PARTY_MEMBER_SWITCH)
    }
